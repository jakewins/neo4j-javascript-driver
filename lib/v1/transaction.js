...
var LambdaTransaction = (function () {
  function LambdaTransaction(tx) {
    _classCallCheck(this, LambdaTransaction);
  ...
  _createClass(LambdaTransaction, [{
    key: 'run',
    value: function run(statement, parameters) {
    ...
    value: function _finish() {
      ...
      var error = undefined;
      this._tailPromise['catch'](function (e) {
        error = e;
      }).then(function () {
        if (error) {
          _this._tx.rollback();
          throw error;
        } else if (_this._state !== 'alive') {
          return _this._tx.rollback();
        } else {
          return _this._tx.commit();
        }
      });
    }
  }]);

  return LambdaTransaction;
})();
